...
class SimpleSmartFilter:
    ...
    def _make_decision(
        self, signals: Dict[str, Any], text: str
    ) -> Tuple[DecisionType, float, RiskLevel, str]:
...
def run_demo():
    ...
    for text, _ in test_cases:
        ...
        risk_levels[result.risk_level.value] = (
            risk_levels.get(result.risk_level.value, 0) + 1
        )
    ...
    print("\n[OK] Демонстрация завершена успешно!")
