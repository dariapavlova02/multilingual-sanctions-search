...
class NormalizationService:
    ...
    def __init__(self):
        ...
        # Merge diminutive sources for downstream processors
        self._combined_diminutive_maps = self._combine_diminutive_maps()

        # Initialize the new processor factory
        self.normalization_factory = NormalizationFactory(
            self.name_dictionaries,
            self._combined_diminutive_maps,
        )
    ...
    def _combine_diminutive_maps(self) -> Dict[str, Dict[str, str]]:
        """Combine dictionary-based and manual diminutive mappings."""
        combined: Dict[str, Dict[str, str]] = {}
        languages = set(self.dim2full_maps.keys()) | set(self.diminutive_maps.keys())
        for lang in languages:
            merged: Dict[str, str] = {}
            for source in (self.dim2full_maps.get(lang, {}), self.diminutive_maps.get(lang, {})):
                for diminutive, full in source.items():
                    merged[diminutive.lower()] = full
            combined[lang] = merged
        return combined
    ...
    async def _normalize_with_factory(
        ...
        try:
            ...
            original_length = metadata.pop('original_length', len(text))
            normalized_length = metadata.pop('final_length', len(normalized_text))
            token_count = metadata.pop('token_count', len(tokens))
    ...
    @lru_cache_with_metrics(maxsize=4096, cache_name="morph_nominal")
    @monitor_performance("morph_nominal")
    def _morph_nominal(self, token: str, primary_lang: str, enable_advanced_features: bool = True, policy_flags: Optional[Dict[str, Any]] = None) -> str:
        """
