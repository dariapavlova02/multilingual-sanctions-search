...
class WordNormalizer:
    ...
    def classify_word(self, word: str) -> WordClassification:
        ...
        if re.match(r"^[A-Za-z]+\.[A-Za-z]+$", word) or re.match(
            r"^[A-Za-z]+\s+Inc\.?$", word
        ):
        ...
        for ending in self.surname_endings["feminine"]:
            if word_lower.endswith(ending):
                if ending in ["ова", "ева", "ина", "ская", "цкая"]:
                    ...
                    masculine_ending = (
                        ending[:-1] + "ов"
                        if ending.endswith("ова")
                        else ending[:-1] + "ев"
                    )
                    normalized = word[: -len(ending)] + masculine_ending
                    self.logger.debug(
                        f"Normalizing feminine surname '{word}' to '{normalized}'"
                    )
                    return normalized
        ...
        for ending in self.surname_endings["masculine"]:
            if word_lower.endswith(ending):
                if ending == "а" and len(word) > 4:
                    ...
                    normalized = word[:-1] + "о"
                    self.logger.debug(
                        f"Normalizing masculine surname '{word}' to '{normalized}'"
                    )
                    return normalized
                elif ending == "ого":
                    # Convert genitive to nominative for masculine surnames
        ...
        for ending in self.surname_endings["ukrainian_masculine"]:
            if word_lower.endswith(ending):
                if ending == "а" and len(word) > 4:
                    ...
                    normalized = word[:-1] + "о"
                    self.logger.debug(
                        f"Normalizing Ukrainian masculine surname '{word}' to '{normalized}'"
                    )
                    return normalized
                elif ending == "ого":
                    # Convert genitive to nominative for Ukrainian masculine surnames
