...
class EmbeddingService:
    ...
    def __init__(self, default_model: str = "sentence-transformers/all-MiniLM-L6-v2"):
        ...
        self.logger.info(
            f"EmbeddingService initialized with default model: {default_model}"
        )
    ...
    def get_embeddings(
        ...
        try:
            ...
            self.logger.info(
                f"Generated embeddings for {len(texts)} texts using model {model_name}"
            )
            return result
    ...
    def _calculate_embedding_similarity(
        self, embedding1: List[float], embedding2: List[float], metric: str
    ) -> float:
    ...
    def get_embedding_statistics(self, embeddings: List[List[float]]) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = {
                ...
                "dimension": (
                    embeddings_array.shape[1] if embeddings_array.size > 0 else 0
                ),
                "mean_norm": float(
                    np.mean([np.linalg.norm(emb) for emb in embeddings])
                ),
                ...
                "sparsity": float(
                    np.mean([np.count_nonzero(emb) / len(emb) for emb in embeddings])
                ),
            }
    ...
    def get_model_info(self, model_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"name": model_name, "error": str(e), "loaded": False}
