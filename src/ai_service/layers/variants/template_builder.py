...
class EntityTemplate:
    ...
    def __post_init__(self):
        ...
        if self.token_variants:
            total_token_variants = sum(
                len(variants) for variants in self.token_variants.values()
            )
...
class TemplateBuilder:
    ...
    def create_entity_template(
        ...
        try:
            ...
            self.logger.info(
                f"Created template for entity: {entity.get('text', '')[:50]}..."
            )
            return template

        except Exception as e:
            self.logger.error(
                f"Failed to create template for entity {entity.get('text', '')}: {e}"
            )
    ...
    def create_batch_templates(
        ...
        self.logger.info(
            f"Created {len(templates)} templates from {len(entities)} entities"
        )
        return templates

    def _generate_search_patterns(
        self, normalized_text: str, variants: List[str]
    ) -> List[str]:
        """Generate search patterns"""
    ...
    def get_template_statistics(
        ...
        stats = {
            ...
            "average_complexity": sum(t.complexity_score for t in templates)
            / len(templates),
            ...
            "confidence_distribution": {
                ...
                "medium": len(
                    [t for t in templates if 0.5 <= t.template_confidence < 0.8]
                ),
        ...
        for template in templates:
            ...
            stats["type_distribution"][entity_type] = (
                stats["type_distribution"].get(entity_type, 0) + 1
            )
