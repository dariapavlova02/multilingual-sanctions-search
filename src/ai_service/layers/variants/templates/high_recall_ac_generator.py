...
class HighRecallACGenerator:
    ...
    def __init__(self):
        ...
        self.document_patterns = {
            ...
            # ИНН РФ - 10 и 12 цифр
            "inn_ru": [
                r"\b(?:ИНН|инн|inn|идентификационный\s+номер)[:\s]*(\d{10})\b",  # 10 цифр с контекстом
                r"\b(?:ИНН|инн|inn|идентификационный\s+номер)[:\s]*(\d{12})\b",  # 12 цифр с контекстом
                r"\b\d{10}\b(?=.*(?:ИНН|инн|inn))",  # 10 цифр в контексте ИНН
                r"\b\d{12}\b(?=.*(?:ИНН|инн|inn))",  # 12 цифр в контексте ИНН
            ...
            # ИНН Украина - 10 цифр
            "inn_ua": [
                r"\b(?:ИНН|інн|inn|ідентифікаційний\s+номер)[:\s]*(\d{10})\b",  # 10 цифр с контекстом
            ...
            # ЕДРПОУ - 8 цифр
            "edrpou": [
                r"\b(?:ЄДРПОУ|едрпou|edrpou|єдиний\s+державний\s+реєстр)[:\s]*(\d{8})\b",  # 8 цифр с контекстом
                r"\b\d{8}\b(?=.*(?:ЄДРПОУ|едрпou|edrpou))",  # 8 цифр в контексте ЕДРПОУ
