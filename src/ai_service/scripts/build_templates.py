...
def build_templates(data: Dict[str, Any], output_dir: str):
    ...
    if "persons" in data:
        ...
        person_templates = template_builder.create_batch_templates(
            data["persons"], entity_type="person"
        )
        ...
        logger.info(
            f"Saved {len(person_templates)} person templates to {person_output}"
        )
    ...
    if "companies" in data:
        ...
        company_templates = template_builder.create_batch_templates(
            data["companies"], entity_type="company"
        )
        ...
        logger.info(
            f"Saved {len(company_templates)} company templates to {company_output}"
        )
    ...
    if "terrorism" in data:
        ...
        terrorism_templates = template_builder.create_batch_templates(
            data["terrorism"], entity_type="terrorism"
        )
        all_templates.extend(terrorism_templates)

        # Save terrorism templates
        terrorism_output = os.path.join(
            output_dir, "terrorism_black_list_templates.json"
        )
        with open(terrorism_output, "w", encoding="utf-8") as f:
            json.dump(terrorism_templates, f, ensure_ascii=False, indent=2)
        logger.info(
            f"Saved {len(terrorism_templates)} terrorism templates to {terrorism_output}"
        )
    ...
    with open(aho_corasick_output, "w", encoding="utf-8") as f:
        for pattern in patterns:
            f.write(pattern + "\n")
    logger.info(
        f"Exported {len(patterns)} patterns for Aho-Corasick to {aho_corasick_output}"
    )
...
def main():
    ...
    try:
        ...
        logger.info(
            f"Template building completed successfully. Created {len(templates)} templates"
        )
