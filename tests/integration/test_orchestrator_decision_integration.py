...
class TestOrchestratorDecisionIntegration:
    ...
    @pytest.mark.asyncio
    async def test_skip_scenario_smartfilter_false(
        self, orchestrator_with_decision_engine
    ):
        ...
        if result.decision:
            ...
            assert result.decision.risk in [
                RiskLevel.SKIP,
                RiskLevel.LOW,
                RiskLevel.MEDIUM,
                RiskLevel.HIGH,
        ...
        for text, description in test_cases:
            ...
            if result.decision:
                ...
                if score >= 0.85:
                    assert (
                        risk == RiskLevel.HIGH
                    ), f"Expected HIGH for score {score}, got {risk}"
                elif score >= 0.65:
                    assert (
                        risk == RiskLevel.MEDIUM
                    ), f"Expected MEDIUM for score {score}, got {risk}"
                else:
                    assert risk in [
                        RiskLevel.LOW,
                        RiskLevel.SKIP,
                    ], f"Expected LOW/SKIP for score {score}, got {risk}"
