...
from src.ai_service.services.normalization_service import (
    NormalizationService
)
from src.ai_service.utils.trace import NormalizationResult
from src.ai_service.exceptions import NormalizationError, LanguageDetectionError
...
class TestNormalizationService:
    ...
    def service(self):
        ...
        with patch('src.ai_service.services.normalization_service.nlp_en'), \
             patch('src.ai_service.services.normalization_service.nlp_ru'), \
             patch('src.ai_service.services.normalization_service.nlp_uk'), \
             patch('src.ai_service.services.normalization_service._nltk_stopwords'), \
             patch('src.ai_service.services.normalization_service.LanguageDetectionService') as mock_lang_service, \
             patch('src.ai_service.services.normalization_service.UnicodeService') as mock_unicode_service:
    ...
    def test_tokenize_text_with_spacy_english(self, service):
        """Test text tokenization with SpaCy for English"""
        # Mock the spacy model in language_configs
        mock_token1 = Mock()
        ...
        # Mock the spacy model directly in the service
        service.language_configs['en']['spacy_model'] = Mock(return_value=mock_doc)

        result = service.tokenize_text("Hello world", "en")

        assert result == ["Hello", "world"]

    def test_tokenize_text_with_spacy_russian(self, service):
        """Test text tokenization with SpaCy for Russian"""
        # Mock the spacy model in language_configs
        mock_token1 = Mock()
        mock_token1.text = "Привет"
        mock_token1.is_space = False
        mock_token2 = Mock()
        ...
        # Mock the spacy model directly in the service
        service.language_configs['ru']['spacy_model'] = Mock(return_value=mock_doc)

        result = service.tokenize_text("Привет мир", "ru")

        assert result == ["Привет", "мир"]

    def test_tokenize_text_with_spacy_ukrainian(self, service):
        """Test text tokenization with SpaCy for Ukrainian"""
        # Mock the spacy model in language_configs
        mock_token1 = Mock()
        mock_token1.text = "Привіт"
        mock_token1.is_space = False
        mock_token2 = Mock()
        mock_token2.text = "світ"
        mock_token2.is_space = False
        ...
        # Mock the spacy model directly in the service
        service.language_configs['uk']['spacy_model'] = Mock(return_value=mock_doc)

        result = service.tokenize_text("Привіт світ", "uk")
    ...
    def test_tokenize_text_fallback_to_nltk(self, service):
        ...
        with patch('src.ai_service.services.normalization_service._word_tokenize') as mock_tokenize:
    ...
    def test_tokenize_text_basic_fallback(self, service):
        ...
        with patch('src.ai_service.services.normalization_service._word_tokenize', None):
            result = service.tokenize_text("Hello world", "en")
    ...
    def test_remove_stop_words_fallback(self, service):
        ...
        with patch('src.ai_service.services.normalization_service._nltk_stopwords', None):
...
class TestNormalizationServiceConfiguration:
    ...
    def test_initialization_without_spacy(self):
        ...
        with patch('src.ai_service.services.normalization_service.SPACY_AVAILABLE', False), \
             patch('src.ai_service.services.normalization_service.nlp_en', None), \
             patch('src.ai_service.services.normalization_service.nlp_ru', None), \
             patch('src.ai_service.services.normalization_service.nlp_uk', None):
    ...
    def test_initialization_without_nltk(self):
        ...
        with patch('src.ai_service.services.normalization_service.NLTK_AVAILABLE', False), \
             patch('src.ai_service.services.normalization_service._nltk_stopwords', None), \
             patch('src.ai_service.services.normalization_service.porter_stemmer', None):
    ...
    def test_initialization_minimal_dependencies(self):
        ...
        with patch('src.ai_service.services.normalization_service.SPACY_AVAILABLE', False), \
             patch('src.ai_service.services.normalization_service.NLTK_AVAILABLE', False):
